...
#define PROC_NAME "fliker_free"

#define PROC_DIR_NAME "fliker_free"
#define MIN_BRIGHTNESS "min_brightness"
#define USE_AOD "use_aod"

struct proc_dir_entry *root_entry;

struct proc_dir_entry *enabled, *minbright, *useaod;

static int show_ff_enabled( struct seq_file *seq, void *v)
{
        seq_printf(seq, "%d\n", (if_fliker_free_enabled()?1:0));
                            ...
                            size_t count, loff_t *pos)
{
    ...
    {
    case '0':
        set_fliker_free(false);
        break;
    
    default:
        set_fliker_free(true);
        break;
    }
    return count;
}
...
static ssize_t my_write_useaod( struct file *file, const char __user *buffer,
                            size_t count, loff_t *pos)
{
    int value;
    value = 0;
...
static int show_useaod( struct seq_file *seq, void *v)
{
    seq_printf(seq, "%d\n", (get_useaod()?1:0));
...
static int my_open_useaod(struct inode *inode, struct file *file)
{
    return single_open(file, show_useaod, NULL);
}
...
static const struct file_operations proc_file_ops_useaod = {
    .owner = THIS_MODULE,
    .open = my_open_useaod,
    .read = seq_read,
    .write = my_write_useaod,
    .llseek = seq_lseek,
    .release = single_release,
};
...
static int __init init( void )
{
    ...
    useaod = proc_create(USE_AOD, 0x0666, root_entry, &proc_file_ops_useaod);
    if ( !enabled && !minbright && !useaod ) {
        return ( -EINVAL );
    }
...
static void __exit cleanup( void )
{
        ...
        remove_proc_entry(USE_AOD, root_entry);
}
